let url = "https://crudcrud.com/api/b1b8a9fd64fd48edb2a72bf3ba8ba43b";
...
btn.addEventListener('click', (e) => {
  ...
  axios.post(url + '/appointmentData', obj)
    .then(response => {
      ...
      .then(res => {
        for (let i = 0; i < res.data.length; i++) {
          if (res.data[i].name === item[0]) {
            axios.delete(url + '/appointmentData/' + res.data[i]._id)
          }
        }
      })
    list.removeChild(li);
